...
function Dpdb(fetchOnline, onDeviceParamsUpdated) {
  ...
  this.dpdb = DPDB_CACHE;
	fetchOnline = false;//added by JM: online database doesn't have my Note 4 "A"
  ...
  if (fetchOnline) {
    ...
    xhr.send();
	  ...
	  console.log('loaded offline DPDB.');
      this.recalculateDeviceParams_();
  }
}
...
Dpdb.prototype.recalculateDeviceParams_ = function() {
  ...
  if (newDeviceParams) {
    ...
    console.log('Failed to recalculate device parameters.'); //JM changed from error to log
  }
};
...
Dpdb.prototype.calcDeviceParams_ = function() {
  ...
  for (var i = 0; i < db.devices.length; i++) {
    ...
    //if (Util.isIOS() != (device.type == 'ios')) continue; //JM temporarily removed to see if this is the issue on android
...
WebVRManager.prototype.isFullscreenMode = function() { //JM added
    return this.mode == Modes.VR || this.mode == Modes.MAGIC_WINDOW;
};
...
//JM added:
WebVRManager.prototype.onVRToggle_ = function(){
    if (this.mode == Modes.VR) {
        this.anyModeToNormal_();
        this.setMode_(Modes.NORMAL);
    } else {
  ...
  this.getDeviceByType_(HMDVRDevice).then(function(hmd) {
    ...
    if (hmd.setFieldOfView) {
      // Calculate the optimal field of view for each eye.
        var leftFOV = this.deviceInfo.getFieldOfViewLeftEye(this.isUndistorted);
        var rightFOV = this.deviceInfo.getFieldOfViewRightEye(this.isUndistorted);
        hmd.setFieldOfView(leftFOV,rightFOV); //JM:On Android chrome, this fails...always uses the default FOV (40)
    }
