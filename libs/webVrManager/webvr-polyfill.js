...
function WebVRPolyfill() {
    ...
    if (!this.isWebVRAvailable()) {
        this.enablePolyfill();
    }  else if  (/firefox/i.test(navigator.userAgent) && 'getVRDevices' in navigator) { //JM added because Firefox believes that VR is enabled if the extension is on but no devices are connected
        navigator.getVRDevices().then(function (theDevices) {
